...
There is also a `.mcp.json` at the repo root, which configures an MCP Postgres server *for Claude Code's own tool use* (connects to `local_dev_db`) — this is agent tooling config, not application code.
